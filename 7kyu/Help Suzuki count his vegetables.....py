...
def count_vegetables(string):
    items = string.split()
    return sorted([(items.count(x), x) for x in vegetables], reverse=True)


test_string = 'potato tofu cucumber cabbage turnip pepper onion carrot celery mushroom potato tofu cucumber cabbage'
print(count_vegetables(test_string))
