...
def weight_counting(number):
    return sum(int(x) for x in str(number))


def order_weight(strng):
    weight_tuples = [(x, weight_counting(int(x))) for x in strng.split()]
    weight_tuples.sort(key=operator.itemgetter(1, 0))
    return ' '.join(list(x[0] for x in weight_tuples))


print(order_weight('  100 200   31 112  110   '))
