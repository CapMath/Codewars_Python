...
of distances between these towns. ls = [50, 55, 57, 58, 60]. John is tired of driving and he says to Mary
...
The function chooseBestSum (or choose_best_sum or ... depending on the language) will take
as parameters t (maximum sum of distances, integer >= 0), k (number of towns to visit, k >= 1)
...
def choose_best_sum(t, k, ls):
    ...
    for x in combinations(ls, k):
        ...
        if s == t:
            return s
        elif nearest_length <= s < t:
            nearest_length = s
    if nearest_length == 0:
        return None
    else:
        return nearest_length
